...
class UserController {
  ...
  async setStatus(user_id, status) {
    await DB("users").update({ status }).where({ id: user_id });
  }
  ...
  async addContact(userId, contactId) {
    ...
    await DB("contacts").insert({ user_id: userId, contact_id: contactId });
  ...
  async removeContact(userId, contactId) {
    await DB("contacts")
      ...
      .and.where("contact_id", contactId)
      .del();
  ...
  async createRoomReletions(roomId, userId, contactId) {
    ...
    if (Array.isArray(contactId)) {
      ...
      await DB("room_relation").insert(newArr);
    } else {
      await DB("room_relation").insert({
        room_id: roomId[0].id,
        user_id: contactId,
  ...
  async searchRoom(contactId, rooms) {
    return await DB("room_relation")
      .select("*")
      .whereIn(
        "room_id",
        rooms.map(({ id }) => id),
      )
      .and.where("user_id", contactId);
  }
  ...
  async addRoomOnTable(userId, type = "privat", room_name) {
    ...
    return await DB("rooms")
      .insert({ user_id: userId, room_name: roomName, type: type })
      .returning("*");
  }
  ...
  async allUserRoomsJoin(userId) {
    ...
    const countdontReadMess = await this.getMessStatus(
      rooms.map(({ id }) => id),
      userId,
      "oneRoom",
    );
    ...
    return rooms.map((room) => {
      ...
      const countMess = countdontReadMess.filter(
        ({ room_id }) => room_id === id,
      );
  ...
  async addLastMess(text, roomId) {
    await DB("rooms").where("id", roomId).update({ last_mess: text });
  }
  ...
  async updateSettings({ idSess, id, nick_name, sity, url_avatar }) {
    ...
    const session = await DB("session").first("sess").where({ sid: idSess });
  ...
  async removeMess(room_id) {
    await DB("messages").select("*").where("room_id", room_id).del();
  ...
  async removeRoom(room_id) {
    await DB("room_relation").where({ room_id }).del();
    await DB("rooms").where({ id: room_id }).del();
  }

  async updateSizeGroup(room_id, users, room_name, user_id) {
    await DB("room_relation")
      .select("*")
      .where({ room_id })
      .whereNot({ user_id })
      .del();
    ...
    await DB("rooms")
      .update({ room_name: room_name.join(",") })
      .where({ id: room_id });
    await DB("room_relation").insert(
      users.map((user_id) => {
        return {
